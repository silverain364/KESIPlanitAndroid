...
class MiniCalendarHolder (
    ...
    private val scheduleApi = retrofit.create(com.example.kesi.data.remote.PersonalScheduleApi::class.java)
    private val groupsScheduleApi = retrofit.create(com.example.kesi.data.remote.GroupScheduleApi::class.java)
    ...
    private suspend fun getOthersScheduleInMonth(date: LocalDate): List<com.example.kesi.data.model.OtherScheduleDto> {
        return suspendCoroutine { continuation ->
            groupsScheduleApi.findOthersInMonth(date.toString(), groupDto.gid).enqueue(object: Callback<List<com.example.kesi.data.model.OtherScheduleDto>> {
                override fun onResponse(p0: Call<List<com.example.kesi.data.model.OtherScheduleDto>>, p1: Response<List<com.example.kesi.data.model.OtherScheduleDto>>) {
                    Log.d("FullCalendarHolder", "others schedule size : ${p1.body()?.size}")

                    continuation.resume(p1.body() ?: emptyList())
                }

                override fun onFailure(p0: Call<List<com.example.kesi.data.model.OtherScheduleDto>>, p1: Throwable) {
    ...
    private suspend fun geGroupScheduleInMonth(date: LocalDate): List<com.example.kesi.data.model.GroupScheduleDto> {
        return suspendCoroutine { continuation ->
            groupsScheduleApi.findInMonth(date.toString()).enqueue(object: Callback<List<com.example.kesi.data.model.GroupScheduleDto>> {
                override fun onResponse(p0: Call<List<com.example.kesi.data.model.GroupScheduleDto>>, p1: Response<List<com.example.kesi.data.model.GroupScheduleDto>>) {
                    Log.d("FullCalendarHolder", "groups schedule size : ${p1.body()?.size}")

                    continuation.resume(p1.body() ?: emptyList())
                }

                override fun onFailure(p0: Call<List<com.example.kesi.data.model.GroupScheduleDto>>, p1: Throwable) {
                    continuation.resume(emptyList())
                }
            })
        }
    ...
    private suspend fun getPersonalScheduleInMonth(date: LocalDate): List<com.example.kesi.data.model.ScheduleDto> {
        return suspendCoroutine { continuation ->
            scheduleApi.findByMonth(date.toString()).enqueue(object: Callback<List<com.example.kesi.data.model.ScheduleDto>> {
                override fun onResponse(p0: Call<List<com.example.kesi.data.model.ScheduleDto>>, p1: Response<List<com.example.kesi.data.model.ScheduleDto>>) {
                    Log.d("FullCalendarHolder", "personal schedule : ${p1.body()?.size}")

                    continuation.resume(p1.body() ?: emptyList())
                }

                override fun onFailure(p0: Call<List<com.example.kesi.data.model.ScheduleDto>>, p1: Throwable) {
                    continuation.resume(emptyList())
                }
            })
        }
    ...
    override fun bind(monthData: MonthData) {
        ...
        scope.launch {
            val personalScheduleDtoListDeferred: Deferred<List<com.example.kesi.data.model.ScheduleDto>> = async {
                getPersonalScheduleInMonth(monthData.date)
            }

            val groupScheduleDtoListDeferred: Deferred<List<com.example.kesi.data.model.GroupScheduleDto>> = async {
                geGroupScheduleInMonth(monthData.date)
            }

            val othersScheduleDtoListDeferred: Deferred<List<com.example.kesi.data.model.OtherScheduleDto>> = async {
