...
import com.example.kesi.data.model.GroupMemberDto
import com.example.kesi.setting.RetrofitSetting

class AddGroupMemberAdapter(val friendsList: ArrayList<FriendsDto>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AddGroupMemberHolder {
        val binding =
            ItemAddGroupMemberBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        ...
        for (i in 0 until items.size()) {
            if (items.valueAt(i)) {
                checkedMembers.add(
                    GroupMemberDto(
                        friendsList[i].email,
                        friendsList[i].nickname,
                        friendsList[i].gender,
                        friendsList[i].imgPath
                    )
                )
            }
        }
