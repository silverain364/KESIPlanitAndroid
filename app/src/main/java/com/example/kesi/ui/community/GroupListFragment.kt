...
class GroupListFragment : Fragment() {
    ...
    val groupList = ArrayList<com.example.kesi.data.model.GroupSimpleDto>()
    ...
    private fun getGroupList() {
        groupApi.getAllGroups().enqueue(object : Callback<List<com.example.kesi.data.model.GroupSimpleDto>> {
            override fun onResponse(p0: Call<List<com.example.kesi.data.model.GroupSimpleDto>>, response: Response<List<com.example.kesi.data.model.GroupSimpleDto>>) {
                groupList.clear()
                response.body()?.let { groupList.addAll(it) }
                adapter.notifyDataSetChanged()
            }

            override fun onFailure(p0: Call<List<com.example.kesi.data.model.GroupSimpleDto>>, p1: Throwable) {
